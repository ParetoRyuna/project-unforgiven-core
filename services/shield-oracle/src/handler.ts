...
export async function handleShieldScoreRequest(body: ShieldScoreBody): Promise<ShieldScoreResult> {
  try {
    if (!body.wallet) {
      return { status: 400, body: { error: 'Missing wallet' } };
    }
    ...
    if (mode === 'verified') {
      ...
      if (!ownership.valid) {
        return {
          status: 400,
          body: { error: ownership.reason ?? 'Invalid attestation ownership' },
        };
      }
    ...
    if (mode === 'verified' && existing && existing > now) {
      return { status: 409, body: { error: 'Proof already used in active window' } };
    }
    ...
    return {
      status: 500,
      body: { error: error instanceof Error ? error.message : 'shield-score failed' },
    };
  }
}
